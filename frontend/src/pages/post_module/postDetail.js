...
import EditorDisplay from "../../components/helper/showEditorContent";
...
const PostDetail = () => {
    ...
    useEffect(() => {
        const fetchAPI = async () => {
            const post = await getDataFromServer(`/api/posts/manage/${id}/`)
            setPostUser(adminUsers.find(user => user.id == post.author))
            setPost(post)
        }
        fetchAPI()
    }, [id])
    const showComment = async (id) => {
        if (!displayComment && !comment.length) {
            setComment(await getDataFromServer(`/api/posts/comments/?post_id=${id}`))
        }
        ...
        <>
            {
                post ? <div className="intro-y news xl:w-3/5 p-5 box mt-8">
                    ...
                    <div className="intro-y text-justify leading-relaxed">
                        <EditorDisplay content={post.content}/>
                    </div>
                    <div
                        ...
                        <div className="flex items-center">
                            ...
                            <div className="mr-3 ml-auto">
                                <div className="font-medium">{postUser.username}</div>
                                , نویسنده
                            </div>
                        </div>
                    ...
                    <div
                        className="intro-y flex text-xs sm:text-sm flex-col justify-center sm:flex-row items-center mt-5 pt-5 border-t border-gray-200 dark:border-dark-5">
                        <button className="btn btn-primary " onClick={() => showComment(post.id)}>
                            {displayComment ? 'بستن کامنت' : 'نمایش کامنت'}
                        </button>
                    </div>
                    {
                        displayComment && <CommentContainer
                            itemComment={comment} itemId={post.id} url={'/api/posts/comments/'}
